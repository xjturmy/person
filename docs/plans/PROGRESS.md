...
> ⚠️ **本看板冻结于 2026-05-10(覆盖到 v2.5)。** 此后的 v2.6~v2.9 迭代未回填本文,实际进展请以项目记忆(`.claude/.../memory/`)与各版本计划为准。下方内容为历史快照,不再逐条更新。
>
> 与当前事实基线(2026-06-28)的脱节点:
> - **公司库**:本文多处写"15 家",现已扩至 **100 家**(沪深 300 + 林奇/格雷厄姆分类,见 project_v100_graham_expansion)。
> - **Dashboard**:本文为 4-tab/多-tab 形态,现为 **v2.9 五导航**(市场&行业 / 选股 / 公司研究 / 决策中心 / 黄金)。
> - **数据库**:本文"数据状态"表列 `peers.duckdb`,但当前 `data/` 实存 7 个库(preson/macro/turnover/etf/gold/decisions/market),**peers.duckdb 缺失待重建**(近期 git 历史重写后未恢复)。
>
> ---
>
