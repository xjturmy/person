...
**P0 拟新增**（见第六节）：`funnel/layers.py` — `expand_industry_to_tickers()` 统一封装上述 join，避免各 Tab 重复逻辑。
...
**P0 拟新增**：`funnel/session.py` 统一读写上述 key，避免魔法字符串散落。
...
## 六、拟新增 `funnel/` 模块（P0 编码阶段）

```
.tools/dashboard/funnel/
    ├── __init__.py
    ├── layers.py      # L0-L4 读取；expand_industry_to_tickers()；get_screener_universe()
    ├── orphans.py     # find_orphan_watchlist()
    └── session.py     # 草稿 key 读写；clear_after_confirm(nav_group)
```

| 函数（草案） | 职责 |
|--------------|------|
| `layers.get_focus_industries()` | 读 L1 |
...
| `session.get_draft(nav)` / `session.clear_draft(nav)` | 草稿生命周期 |
